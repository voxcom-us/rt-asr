...
fn warmup(
    ...
    let mask = asr_core::StreamMask::new(vec![true; state.batch_size()], &dev)?;
    for _ in 0..2 {
...
impl BatchedAsrInner {
    fn start_model_loop(
        ...
        crate::utils::spawn_blocking("model_loop", move || {
            ...
            loop {
                ...
                if with_data > 0 {
                    let mask = asr_core::StreamMask::new(mask, &dev)?;
                    let pcm =
    ...
    fn post_process(
        &self,
        asr_msgs: Vec<asr_core::asr::AsrMsg>,
        step_idx: usize,
        markers: &mut BinaryHeap<Marker>,
        mask: &asr_core::StreamMask,
...
impl BatchedAsr {
    pub fn new(
        ...
        let lm = asr_core::lm::LmModel::batched(batch_size, &asr.model, lm_vb)?;
        let audio_tokenizer = {
            let vb = unsafe {
            ...
            asr_core::mimi::Mimi::batched(batch_size, cfg, vb)?
        };
