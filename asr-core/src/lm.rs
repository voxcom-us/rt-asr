...
impl LmModel {
    ...
    pub fn new_(
        ...
        if let Some(ExtraHeadsConfig { num_heads, dim }) = cfg.extra_heads {
            let extra_heads_vb = vb.pp("extra_heads");
            for i in 0..num_heads {
                let head_vb = extra_heads_vb.pp(i);
                match linear(d_model, dim, false, head_vb) {
                    Ok(extra_head) => extra_heads.push(extra_head),
                    Err(err) => {
                        let err_msg = err.to_string();
                        if err_msg.contains("cannot find tensor") {
                            warn!(target: "lm", head = i, "missing extra head weights, skipping remaining heads");
                            break;
                        } else {
                            return Err(err);
                        }
